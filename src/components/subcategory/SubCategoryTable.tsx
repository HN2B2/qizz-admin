...
const SubCategoryTable = ({
  ...
  const handleUpdateSubCategory = (subCategory: SubCategory, index: number) => {
    modals.open({
        ...
        <Table.Tbody>
          {renderList.map((subCategory, index) => (
            <Table.Tr key={subCategory.id}>
              ...
              <Table.Td>
                <Text lineClamp={1}>{subCategory.description}</Text>
              </Table.Td>
              <Table.Td>
                <Text lineClamp={1}>{convertDate(subCategory.createdAt)}</Text>
              </Table.Td>
              <Table.Td>
                <Group gap={4}>
                  <ActionIcon
                    variant="default"
                    onClick={() => handleUpdateSubCategory(subCategory, index)}
                  >
                    <IconEdit size={"1rem"} stroke={1.5} />
                  </ActionIcon>
                  <ActionIcon
                    variant="default"
                    onClick={() => handleDeleteSubCategoryModal(index)}
                  >
