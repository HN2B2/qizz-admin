...
const CreateNewSubCategoryModal = ({
  ...
  const handleCreateNewSubCategory = async () => {
    ...
    try {
      const data: SubCategory = await instance
        .post(`subcategories/${categoryId}`, {
          json: {
            name: newSubCategoryForm.values.name,
            description: newSubCategoryForm.values.description,
          },
        })
        .json();
      modals.closeAll();
  ...
  return (
    <Stack>
      ...
      <Group justify="end">
        ...
        <Button color="teal" type="submit" onClick={handleCreateNewSubCategory}>
          Create
        </Button>
      </Group>
    </Stack>
